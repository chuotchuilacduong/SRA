...
class LinearRAG:
    ...
    def add_adjacent_passage_edges(self):
        ...
        Inputs to ``index()`` are passages prefixed with ``"<int_idx>:"``; this
        method parses that prefix and chains passages in numeric order. Useful
        when corpus order reflects topical proximity (Wikipedia paragraphs in
        an article, clustered skills). Skip via ``enable_passage_adjacency=False``
        when corpus order is arbitrary — weight=1.0 dominates the random walk
