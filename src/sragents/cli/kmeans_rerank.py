"""``sragents kmeans-rerank`` — K-means cluster-aware reranking of BM25 results.

Reads a retrieval JSON produced by ``sragents retrieve``, reranks each
query's candidates by blending BM25 scores with semantic cluster affinity,
and writes a new retrieval JSON in the same schema.
"""
...
from sragents.retrieve.kmeans_rerank import KMeansReranker
...
def add_parser(subparsers) -> None:
    p = subparsers.add_parser(
        "kmeans-rerank",
        help="K-means cluster-aware reranking of a retrieval result file (stage 1)",
        description=(
            "Reranks BM25 candidates by blending BM25 scores with the cosine "
            "similarity between the query embedding and the K-means centroid of "
            "each candidate skill.  final_score = alpha*bm25 + (1-alpha)*cluster_affinity"
        ),
    )
    ...
    p.add_argument("--n-clusters", type=int, default=300,
                   help="Number of K-means clusters (default: 300)")
    p.add_argument("--alpha", type=float, default=0.5,
                   help="BM25 weight in [0,1]; (1-alpha) goes to cluster affinity (default: 0.5)")
...
def run(args) -> None:
    ...
    print(
        f"Building K-means index: K={args.n_clusters}, alpha={args.alpha}...",
        flush=True,
    )
    reranker = KMeansReranker(
    ...
    result = RetrievalResults(
        retriever=f"kmeans_rerank_{source['metadata'].get('retriever', 'unknown')}",
        top_k=args.top_k,
